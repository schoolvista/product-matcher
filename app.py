...
st.markdown(
    '<div style="font-size:1.35rem;font-weight:600;line-height:1.3;margin:0 0 0.25rem 0;">'
    "Product substitute lookup"
    "</div><br><br>",
...
def build_equivalent_cli_command(
    *,
    ...
    if (target_brand or "").strip():
        parts += ["--target-brand", shlex.quote(target_brand.strip())]
...
def _confidence_sort_rank(conf: object) -> int:
    """Table sort order: high, medium, low, then any other / missing value."""
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "| Target brand (optional) | `--target-brand` |\n"
...
with c3:
    target_brand = st.text_input(
        ...
        value="",
        placeholder="e.g. Haneco — leave blank for any brand",
        help="CLI: `--target-brand`. Empty: search the open web for substitutes from any manufacturer.",
    )
...
if run:
    if not (source_brand or "").strip() or not (product_code or "").strip():
        st.warning("Fill in source brand and product code.")
    else:
        with st.status("Running Tavily + Claude (may take 30–90 seconds)…", expanded=True) as status:
            status.markdown(
                "**Progress** — each line shows start/end timestamps (local time) and duration."
            )
            try:
                ...
                payload = m.run_single_lookup(
                    ...
                    verbose=False,
                    step_callback=lambda msg: status.markdown(msg),
                )
        ...
        if isinstance(prof, dict) and prof.get("summary"):
            with st.expander("Extracted source profile", expanded=False):
                st.json(prof)
                ...
                rows.append(
                    {
                        ...
                        "Brand": (s.get("matched_brand") or "").strip(),
                        "Confidence": s.get("confidence", ""),
                        "Reasoning": s.get("reasoning", ""),
                        "URLs": "; ".join(s.get("cited_urls") or []),
                    }
                )
            rows.sort(key=lambda r: _confidence_sort_rank(r["Confidence"]))
            st.dataframe(rows, use_container_width=True, hide_index=True)
        else:
            st.info("No substitutes returned (check evidence or try adding a description).")
